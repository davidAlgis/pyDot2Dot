...
def calculate_discrete_curvature(points):
    """
    Calculate discrete curvature at each point based on vector cross-products.
    """
    kappa = []
    for i in range(1, len(points) - 1):
        p1, p2, p3 = np.array(points[i - 1]), np.array(points[i]), np.array(
            points[i + 1])
        v1, v2 = p1 - p2, p3 - p2
        cross_product = np.cross(v1, v2)
        norm_v1, norm_v2 = np.linalg.norm(v1), np.linalg.norm(v2)
        sin_angle = cross_product / (norm_v1 * norm_v2)
        curvature = abs(sin_angle) / (norm_v1 +
                                      norm_v2) if norm_v1 + norm_v2 != 0 else 0
        kappa.append(curvature)
    return [0] + kappa + [0]  # Curvature at endpoints set to 0


def select_top_k_percent_points(points, kappa, top_percent=5):
    """
...
def prune_points_arc_length(points, min_arc_length):
    """
...
def contour_to_linear_paths(contours,
    ...
    for contour in contours:
        ...
        total_arc_length = calculate_arc_length(points)

        # Step 2: Prune points based on a fraction of the total arc length
        pruned_points = prune_points_arc_length(points,
                                                total_arc_length * 0.005)

        # Step 3: Calculate curvature on pruned points
        curvature = calculate_discrete_curvature(pruned_points)

        # Step 4: Identify top 10% points by curvature
        top_curvature_points = select_top_k_percent_points(pruned_points,
                                                           curvature,
                                                           top_percent=10)
